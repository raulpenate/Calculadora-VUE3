...
export default {
  ...
  methods: {
    ...
    igual() {
      //REGEX To replace any, ^ start with x, -, /,  + simbol, \s for a space, gm global multiline
      let primerTermino = parseFloat(this.pantallainterna);
      let segundoTermino = parseFloat(this.pantallasuperior.replace(/^(\x|\-|\/|\+)\s/gm, ''));
      let signo = segundoTermino.slice(0,1);
      this.pantallasuperior = null;
      this.pantallainterna = hacerOperacion(primerTermino, segundoTermino, signo)
    },
  },
};
</script>
