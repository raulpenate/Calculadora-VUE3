    ...
    <div class="carcasa">
      <div class="pantalla">
        <div class="datosanteriores">
          <label class="pantallasuperior">{{ pantallasuperior }}</label>
        </div>
        <input
...
export default {
  ...
  methods: {
    ...
    hacerOperacion(primerTermino, segundoTermino, signo){
      return (signo == "+") ? (primerTermino + segundoTermino) :
             (signo == "-") ? (primerTermino - segundoTermino) :
             (signo == "x") ? (primerTermino * segundoTermino) :
             (signo == "/") ? (primerTermino / segundoTermino) : "Signo no encontrado";
    },
    igual(){
      //REGEX To find any, ^ start with x, -, /,  + simbol, \s for a space, gm global multiline
      let wea = this.pantallasuperior.search(/^(\x|\-|\/|\+)\s/gm);
      console.log(wea);
    }
  },
};
